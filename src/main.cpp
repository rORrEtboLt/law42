...
bool init()
{
	...
	window.create("Law 42", SCREEN_WIDTH, SCREEN_HEIGHT);
	...
	// Render a background image or animated backround grass image
	...
	backgroundSfx = Mix_LoadWAV("res/sounds/background.wav");
	Mix_PlayChannel(-1, backgroundSfx, -1);
	return true;
}
...
void gameLoop()
{
 	...
 	while (SDL_PollEvent(&event)) 
 	{
    	...
    	case SDL_MOUSEBUTTONDOWN:
    	{
    		...
    		else 
    		{
				...
				// Jump functionality not required.
				// if (player.jump())
				// 		{
	...
	if (mainMenu)
	{
		...
		{
			window.clear();
			window.renderCenter(0, sin(SDL_GetTicks()/100) * 2 - 4, "RORRET GAMES", font24, white);
			window.display();
		}
		else 
		{
